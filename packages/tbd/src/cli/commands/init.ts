  ...
  async run(options: InitOptions): Promise<void> {
    ...
    await this.execute(async () => {
      ...
      await ensureGitignorePatterns(join(cwd, TBD_DIR, '.gitignore'), [
        ...
        '# that could not be pushed to the tbd-sync branch (e.g. due to HTTP 403',
        '# from git branch restrictions). The ENTIRE POINT of this directory is that',
        '# it gets committed to your working branch so the data survives across',
        '# sessions and can be synced later.',
        '#',
