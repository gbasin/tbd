...
| --- | --- | --- |
| `` | `{% tag attr="val" %}` | Same tag name in both |
| `` | `{% /tag %}` | Closing tags |
| `` | `{% tag /%}` | Self-closing: `/` before `-->` |
| `` | `{% #id %}` | ID annotations |
...
1. Uses either comment syntax (``) or tag syntax (`{% form ... %}`)
...
- *required:* Comment-to-tag transformation MUST only occur **within** the form tag
  boundaries (between the opening `` and closing ``)

- *required:* HTML comments outside the form tag MUST pass through unchanged, even if
  they match Markform tag names (e.g., `` before the form tag)
